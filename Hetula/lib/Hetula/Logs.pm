...
@THROWS Hetula::Exception::Log::NotFound
...
sub getLog {
  ...
  Hetula::Exception::Organization::NotFound->throw(error => 'No log found with params "'.Data::Dumper::Dumper($args).'"') unless $o;
  return $o;
}
...
sub searchLogs {
  ...
  Hetula::Exception::Log::NotFound->throw(error => 'No log found with params "'.Data::Dumper::Dumper($args).'"') unless scalar(@l);
  return \@l;
}
