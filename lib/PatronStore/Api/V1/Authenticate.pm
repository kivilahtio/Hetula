...
sub route {
  ...
  eval {
    ...
    unless ($userid) {
        $userid = _passwordAuthentication($c->req->param('username'), $c->req->param('password'));
    }
    unless ($userid) {
        ($status, $message) = (403, 'Password authentication failed');
        return; #Return from eval, not the subroutine you silly!
    }
